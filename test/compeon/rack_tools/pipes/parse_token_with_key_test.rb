...
require 'compeon/rack_tools/pipes/parse_token_with_key'

module Compeon
  module RackTools
    module Pipes
      class ParseTokenWithKeyTest < Minitest::Test
        AUTH_KEY = OpenSSL::PKey::RSA.new(512)

        def pipe
          Compeon::RackTools::Pipes::PARSE_TOKEN_WITH_KEY.call(key: AUTH_KEY.public_key)
        end
        ...
        def test_parse_token_ok
          ...
          result = pipe.call(request: request)
        ...
        def test_parse_no_token
          assert_raises Compeon::RackTools::UnauthorizedError do
            pipe.call(request: Rack::Request.new({}))
          end
        end
